...
export function LoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.next) {
        window.location.href = callbackUrl !== "/app" ? callbackUrl : data.next;
      }
    } finally {
      setSubmitting(false);
    }
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <Link href="/forgot-password" title="Coming Soon" className="text-xs font-semibold text-accent-primary hover:text-accent-primary-hover transition-colors">
              Forgot password?
            </Link>
          </div>
          <input
